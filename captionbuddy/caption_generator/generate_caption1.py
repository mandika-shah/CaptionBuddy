...
MODEL_PATH = os.path.join(settings.BASE_DIR, 'Flickr','model_13.h5')
WORD_TO_IDX_PATH = os.path.join(settings.BASE_DIR, 'Flickr','word_to_idx.pkl')
IDX_TO_WORD_PATH = os.path.join(settings.BASE_DIR,'Flickr', 'idx_to_word.pkl')

# Read the files word_to_idx.pkl and idx_to_word.pkl to get the mappings between word and index
word_to_index = {}
with open ("WORD_TO_IDX_PATH", 'rb') as file:
    word_to_index = pd.read_pickle(file, compression=None)

index_to_word = {}
with open ("IDX_TO_WORD_PATH", 'rb') as file:
...
# Generate Captions for a random image Using Greedy Search Algorithm
...
# A function, which inputs an image and returns its encoding (feature vector)
